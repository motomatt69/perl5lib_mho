...
#    
#    my $m_db = $self->{m_db};
#    my $apb_hp_bmf = PPEEMS::SHOPDWG::apb_hp_bmf->new('m_db' => $m_db);
#    $self->{apb_hp_bmf} = $apb_hp_bmf;
#    my $m_shop_pdf = PPEEMS::SHOPDWG::m_shop_pdf->new('m_db' => $m_db);
#    $self->{m_shop_pdf} = $m_shop_pdf;
#    my $m_b3d_imp = PPEEMS::SHOPDWG::m_b3d_imp->new();
#    $self->{m_b3d_imp} = $m_b3d_imp;
#    my $m_ftpliste = PPEEMS::SHOPDWG::m_ftpliste->new();
...
#    
    ...
    #   
    #    $self->get_tzdata($ts, $hpos, $blatt, $rev);
    #    my %tzdata = %{$self->zdata()};
    #    my %apbposs = %{$tzdata{apbposs}};
    #           
    #    $self->selteilzng($selteilzng);
    #    
