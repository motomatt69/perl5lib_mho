...
sub readbpslist{
    ...
    my (%subdwgs, $hpos, $tspos);
    for my $l (@lines){
        if ($l->[0] eq 'H'){
            my $ts = $l->[1];
            $hpos = $l->[2];
            $tspos = $ts * 1000000 + $hpos;
            $subdwgs{$tspos} = undef;
            next
        }
        elsif ($l->[0] eq 'W'){
            my $wpos = $l->[1];
            if ($wpos == $hpos){
                $subdwgs{$tspos}->{$wpos}->{typ} = 'bt';
                $subdwgs{$tspos}->{$wpos}->{bt} = $hpos;
            }
            else{    
                $subdwgs{$tspos}->{$wpos}->{typ} = 'abt';
                $subdwgs{$tspos}->{$wpos}->{bt} = $hpos;
            }
        }   
    }
    $self->{subdwgs} = \%subdwgs;
}

sub get_sizes{
    my ($self) = @_;
    
    my @hps = sort keys %{$self->{subdwgs}};
    my %hp_sizes;
