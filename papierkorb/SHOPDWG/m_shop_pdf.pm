package PPEEMS::DWG::SHOP::m_shop_pdf;
use strict;
use Offlmod::offlmod;
use PPEEMS::NEST::nest;
use PDF::API2;
...
has 'nest'        => (isa => 'Object',   is => 'ro', required => 0);
has 'wzdata'      => (isa => 'HashRef', is => 'rw', required => 0);
...
sub BUILD{
    ...
    $self->{nest}     = PPEEMS::NEST::nest->new();
...
sub pdf_creation{
    ...
    my @selteilzngpaths = @$selteilzngpaths;
    $self->{selteilzngpaths} = (\@selteilzngpaths);
    
    my ($anr, $headernr) = ('090477', 2);
    
    my %wzdata = %{$self->wzdata()};
...
sub _nest_pdf{
    ...
    for my $p (@sorted_multiblatt){
        ...
        $self->{nest}->teilzngsizes(\%pdfsize);
        my $frm = $self->_rahmenformat_ermitteln();
        
        $self->{nest}->frm($frm);
        #Schachtelvorgang
        $self->{nest}->nest();
        push @zng, (@{$self->{nest}->zng()});       
    }
    ...
    if (@sorted_singleblatts){
        ...
        $self->{nest}->teilzngsizes(\%maxpdf);
        my $frm = $self->_rahmenformat_ermitteln();
        $self->{nest}->frm($frm);
        $self->{nest}->teilzngsizes(\%pdfsizes);
...
sub _rahmenformat_ermitteln{
    ...
    for my $i (0..$#a0xfrms){
        ...
        $maxfrm->{pw} = $pw;
        $maxfrm->{ph} = $ph;
        $self->{nest}->frm($maxfrm);
        my $nest = $self->{nest}->nest();
