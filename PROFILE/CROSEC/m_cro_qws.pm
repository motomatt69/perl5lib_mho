package PROFILE::CROSEC::m_cro_qws;
use strict;
...
sub calc_plates{
    ...
    #Steg1
    my @p4xy = ($p0xy[0], $p0xy[1] + $tg * $fac );
    my @p5xy = ($p0xy[0] + $ts * $fac, $p0xy[1] + $tg * $fac );
    my @p6xy = ($p0xy[0] + $ts * $fac, $p0xy[1] + ($h - $tg) * $fac);
    my @p7xy = ($p0xy[0] , $p0xy[1] + ($h - $tg) * $fac);
    my @steg1 =  (@p4xy, @p5xy, @p6xy, @p7xy);
    ...
    my @ofl =  (@p8xy, @p9xy, @p10xy, @p11xy);
     ...
     #Steg2
    my @p12xy = ($p0xy[0] + ($b - $ts) * $fac, $p0xy[1] + $tg * $fac );
    my @p13xy = ($p0xy[0] + $b * $fac, $p0xy[1] + $tg * $fac );
    my @p14xy = ($p0xy[0] + $b * $fac, $p0xy[1] + ($h - $tg) * $fac);
    my @p15xy = ($p0xy[0] + ($b - $ts) * $fac, $p0xy[1] + ($h - $tg) * $fac);
    my @steg2 =  (@p12xy, @p13xy, @p14xy, @p15xy);
    
    #S-Naht Punkte;
    my @snaht_pkte = (\@p3xy, \@p13xy, \@p14xy, \@p7xy); 
    $self->{snaht_pkte} = \@snaht_pkte;
        
    my @pkte = (\@p0xy, \@p1xy, \@p2xy, \@p3xy,
                \@p4xy, \@p5xy, \@p6xy, \@p7xy,
                \@p8xy, \@p9xy, \@p10xy, \@p11xy,
                \@p12xy, \@p13xy, \@p14xy, \@p15xy);
    
    $self->{pkte} = \@pkte;
    
    my @plates = (\@ufl, \@steg1, \@ofl, \@steg2);     
...
sub calc_snaht{
    ...
    my @zw3 = ($zw * (1), 0, 0, $zw * (-1));
    my @zw13 = ($zw * (-1), 0, 0, $zw * (-1));
    my @zw14 = ($zw * (-1), 0, 0, $zw * (1));
    my @zw7 = ($zw * (1), 0, 0, $zw * (1));
    
    my @zws = (\@zw3, \@zw13, \@zw14, \@zw7);
...
sub calc_vals{
    ...
    my %vals;
    $vals{bez} = 'QWS'.$h.'*'.$b.'*'.$ts.'*'.$tg;
    $vals{h} = $h;
    $vals{b} = $b;
    $vals{ts} = $ts;
    $vals{tg} = $tg;
    my $G = (2 * $b * $tg + 2 * ($h - 2 * $tg) * $ts) * 0.00785;
    $G = sprintf "%.2f", $G;
    $vals{G} = $G;
    my $U = (2 * $h + 2 * $b) / 1000;
    $U = sprintf "%.2f", $U;
    $vals{U} = $U;
    my $A = 2*($b * $tg) + 2 * ($h - 2 * $tg) * $ts;
    $vals{A} = $A;
    my $ASteg = ($h - $tg) * $ts * 2;
    $vals{ASteg} = $ASteg;
    
    my $Iy = (1 / 12 * $h**3 * $b
              - 1 / 12 * ($h - 2 * $tg)**3 * ($b - 2 * $ts)) / 10000;
    $Iy = sprintf "%.0f", $Iy;
    $vals{Iy} = $Iy;
    my $Wy = $Iy * 2 / ($h / 10);
    $Wy = sprintf "%.0f", $Wy;
    ...
    my $Sy =   1 / 2 * ($b * $tg * 1 / 2 * ($h - $tg)
                        + 2 * $ts * 1 / 8 * ($h - 2 * $tg)**2) / 1000;
    $Sy = sprintf "%0.f", $Sy;
    $vals{Sy} = $Sy;
    my $Sz =   1 / 2 * ($h * $ts * 1 / 2 * ($b - $ts)
                        + 2 * $tg * 1 / 8 * ($b - 2 * $ts)**2) / 1000;
    $Sz = sprintf "%0.f", $Sz;
    $vals{Sz} = $Sz;
    
    my $fyd = 218.2;
    my $Vzd235 = 2 * ($h - $tg) * $ts * $fyd / sqrt (3) / 1000;
    $Vzd235 = sprintf "%0.f", $Vzd235;
    $vals{Vzd235} = $Vzd235; 
    
    my $Vzd355 = 2 * ($h - $tg) * $ts * 1.5 * $fyd / sqrt (3) / 1000;
    $Vzd355 = sprintf "%0.f", $Vzd355;
    $vals{Vzd355} = $Vzd355;
    
    my $Syfl = 1 / 2 * ($b * $tg) * ($h  - $tg) / 2 / 1000;
    $vals{Syfl} = $Syfl;
    my $sy = $Iy / $Sy;
