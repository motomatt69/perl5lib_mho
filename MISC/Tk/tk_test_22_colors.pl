...
my @colors = qw/gray60 gray70 gray80 gray85 gray90
...
my $canv1=$mw->Canvas(-width=>1800,
                      -height=>900,)->pack();

my ($zeile, $spalte);
for ($zeile=0; $zeile <= 850; $zeile +=50) {
    for ($spalte=0; $spalte <= 1700; $spalte +=100) {
        my $colorNr=($zeile/50)*18+$spalte/100;
        if ($colorNr > 317) {last};
        $canv1->createRectangle($spalte,$zeile,$spalte+100,$zeile+50, -fill=>$colors[$colorNr]);
        $canv1->createText($spalte+10,$zeile+40, -text => $colors[$colorNr],
                                                -anchor=>'sw');
    }
}
